...
$('#myButton').on('click', function () {
    if ($('#target').val() != '') {
        var rpm = $('#target').val();
            ...
            if ($('.gasoline-ticks').css('visibility') == 'hidden') {
                if (rpm > 6000) {
                    rpm = 6000;
                } else if (rpm < 0) {
                    rpm = 0;
                }
            } else if ($('.gasoline-ticks').css('visibility') == 'visible') {
                if (rpm > 8000) {
                    rpm = 8000;
                } else if (rpm < 0) {
                    rpm = 0;
...
$('#gasl').on('click', function() {   
    $('.gasoline-ticks').css('visibility','visible');
     
    $('#target').attr('placeholder','0-8000');

    $('#critical').removeClass('redline1');
    $('#critical').addClass('redline2');

   
        lpf.smoothing = 0.01;
        var tempTargetRpm2;
        var lastArrowAngle2;   
        timeOutVar = setTimeout( function(){moveArrow(-120, lastArrowAngle2, tempTargetRpm2);}, 25 );
    
    
});

$('#dsl').on('click', function() {
    $('.gasoline-ticks').css('visibility','hidden');

    $('#target').attr('placeholder','0-6000');

    $('#critical').removeClass('redline2');
    $('#critical').addClass('redline1');
